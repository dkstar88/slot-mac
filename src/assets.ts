...
export function loadAllAssets(): Promise<
  | { asset: { src: string; alias: string }; texture: any; success: boolean }
  | { asset: { src: string; alias: string }; error: any; success: boolean }
>[] {
  ...
  const allAssets = [...glyphsAssets, ...uiAssets, ...animationAssets];

  const loadPromises = allAssets.map((asset) => {
    return Assets.load(asset)
      .then((texture) => {
        logger.info(`Successfully loaded texture: ${asset.src}`);
        return { asset, texture, success: true };
      })
      .catch((error) => {
        logger.error(`Failed to load texture: ${asset.src}`, error);
