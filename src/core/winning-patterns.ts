...
import {
  WinCombinationType,
  WinningPattern,
} from "../types/winning-combinations";
...
export function* checkSymbolsMatch(
  board: GlyphInstance[][],
  patternMatrix: number[][],
): IterableIterator<GlyphInstance[]> {
  ...
  for (const sliced of sliceArray(
    board,
    patternMatrix.length,
    patternMatrix[0].length,
  )) {
 ...
 *
 */
export function transformBoardSymbolsToMatrix(
  board: GlyphInstance[][],
): GlyphInstance[][] {
...
export function detectWins(board: GlyphInstance[][]): Win[] {
  ...
  const wins: Win[] = winningCombinations.map(({ pattern, symbols }) => {
    const symbolValue = Math.floor(
      symbols.reduce((acc, symbol) => acc + symbol.glyph.payoutValue, 0) /
        symbols.length,
    );
...
export function calculatePayout(
  ...
  const totalPayout =
    wins.reduce((acc, win) => acc + win.totalValue, 0) * currentMultiplier;
...
class _WinningPatternsManager {
  ...
  public incGroupMultiplier(group: string, value: number): void {
    for (const pattern of this.winningPatterns.filter(
      (p) => p.group === group,
    )) {
