...
import {
  detectWinningCombinations,
  transformBoardSymbolsToMatrix,
...
import { WinningPattern } from "../types/winning-combinations";
/**
...
export class GameBoard extends PIXI.Container {
  ...
  private slotFrame: PIXI.Container;
  /** Array of reels */
  private reels: Reel[] = [];
  ...
  constructor(config: Partial<GameBoardConfig> = {}) {
    ...
    this.slotFrame = new PIXI.Sprite(PIXI.Assets.get("slots"));
    this.slotFrame.position.set(
      this.config.width / 2 - boardWidth / 2 - 4,
      this.config.height / 2 - boardHeight / 2 + MAIN_CONFIG.board.y,
    );
    this.slotFrame.width = boardWidth;
    this.slotFrame.height = boardHeight;
    ...
    this.reelsContainer.position.set(
      this.config.width / 2 -
        (this.config.columns *
          (this.config.glyphSize + this.config.reelSpacing)) /
          2,
  ...
  private initReels(): void {
    ...
    for (let col = 0; col < this.config.columns; col++) {
      ...
      const reelConfig: Partial<ReelConfig> = {
        ...
        spinStartDelay:
          (this.config.reelConfig?.spinStartDelay || 0) + col * 100, // Stagger start
  ...
  private subscribeToEvents(): void {
    ...
    eventManager.subscribe(
      GameEventType.REEL_STOPPED,
      (event: ReelStoppedEvent) => {
    ...
    eventManager.subscribe(
      GameEventType.WIN_DETECTED,
      (event: WinDetectedEvent) => {
        this.onWinDetected(event.pattern, event.symbols);
      },
    );
  }

  private onWinDetected(
    pattern: WinningPattern,
    symbols: GlyphInstance[],
  ): void {
    logger.debug("GameBoard: Win detected", pattern, symbols);
   ...
   */
  public spin(): void {
    logger.debug("GameBoard: spin called");

    if (this.isSpinning) {
      logger.debug("GameBoard: Already spinning, ignoring spin call");
      return;
    }
    ...
    logger.debug(`GameBoard: Starting to spin ${this.reels.length} reels`);
    for (let i = 0; i < this.reels.length; i++) {
      logger.debug(`GameBoard: Spinning reel ${i}`);
      this.reels[i].spin();
    }
  }
  ...
  private detectWins(): void {
    logger.debug("GameBoard: Detecting wins", this.board);
  ...
  public initSymbols(): void {
    ...
    logger.debug("GameBoard: Initializing symbols", this.board);
